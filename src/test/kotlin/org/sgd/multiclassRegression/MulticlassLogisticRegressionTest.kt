...
private const val DATASET = "covtype"

private val dataset by lazy { loadMulticlassDataSet(File(baseDir, DATASET), File(baseDir, "$DATASET.t")) }
...
class RunRegressionTask(
    ...
    private val trainLoss = MulticlassLogisticRegressionModel(dataset.first, dataset.third, features)
    private val testLoss = MulticlassLogisticRegressionModel(dataset.second, dataset.third, features)
...
class MulticlassLogisticRegressionTest {
    ...
    fun run() {
        ...
        val p = params[DATASET]!!
        val solver = ClusterParallelSGDSolver(p["learningRate"]!!, 1, p["stepDecay"]!!, 1)
